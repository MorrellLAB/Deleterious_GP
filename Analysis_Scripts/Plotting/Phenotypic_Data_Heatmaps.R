...
phenotype_map <- function(yl, dat, variable) {
    #   Trim the data down to just the year-location we are interested in
    year_loc <- dat[dat$trial == yl, c("row", "column", "line_name", "Type", variable)]
    ...
    for(i in 1:nrow(year_loc)) {
        ...
        yld <- as.numeric(year_loc[i, variable])
...
plot_hmp <- function(yl, phenmaps, variable) {
    ...
    #   Make the plot
    pdf(
        file=paste(yl, "_Raw", variable, ".pdf", sep=""),
        width=10,
        height=8)
    #   Plot the heatmap
    image(
        ...
        xlab="Column",
        ylab="Row",
        main=paste(yl, "Raw", variable, "Data", sep=" "),
...
yield_maps <- lapply(yld_trials, phenotype_map, raw_yield, "yld_bu")
names(yield_maps) <- yld_trials
don_maps <- lapply(don_trials, phenotype_map, raw_don, "DON")
