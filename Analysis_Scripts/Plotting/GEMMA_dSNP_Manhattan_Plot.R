# Make a Manhattan plot of -log(P_lrt) from the GEMMA results for just dSNPS.

# Read the GEMMA association results
yld <- read.table("/Users/tomkono/Dropbox/GitHub/Deleterious_GP/Results/GEMMA/Yield_Deleterious_GEMMA.out.txt.gz", header=TRUE)
don <- read.table("/Users/tomkono/Dropbox/GitHub/Deleterious_GP/Results/GEMMA/DON_Deleterious_GEMMA.out.txt.gz", header=TRUE)
hgt <- read.table("/Users/tomkono/Dropbox/GitHub/Deleterious_GP/Results/GEMMA/Height_Deleterious_GEMMA.out.txt.gz", header=TRUE)
...
pdf(file="GEMMA_dSNP_Manhattan.pdf", height=4, width=8)
# png(file="GEMMA_dSNP_Manhattan.png", res=150, height=600, width=1200)
par(mfrow=c(3, 1), mar=c(1, 3, 0.75, 0.1), mgp=c(2, 1, 0))
chroms <- c("Chr1H", "Chr2H", "Chr3H", "Chr4H", "Chr5H", "Chr6H", "Chr7H")
# Make a plot for Yield
plot(0, type="n", axes=FALSE, ylim=c(0, 5), xlim=c(0, 4600), xlab="", ylab="-log10(P)", main="Yield (kg/ha)")
sapply(
    seq_along(chroms),
    function(x) {
        cname <- chroms[x]
...
abline(h=-log10(0.05/nrow(yld_plt)), col="red", lwd=0.75, lty=3)
...
# Make a plot for DON
plot(0, type="n", axes=FALSE, ylim=c(0, 5), xlim=c(0, 4600), xlab="", ylab="-log10(P)", main="DON (ppm)")
sapply(
    seq_along(chroms),
    function(x) {
        cname <- chroms[x]
...
abline(h=-log10(0.05/nrow(don_plt)), col="red", lwd=0.75, lty=3)
...
plot(0, type="n", axes=FALSE, ylim=c(0, 5), xlim=c(0, 4600), xlab="", ylab="-log10(P)", main="Height (cm)")
sapply(
    seq_along(chroms),
    function(x) {
        cname <- chroms[x]
...
abline(h=-log10(0.05/nrow(hgt_plt)), col="red", lwd=0.75, lty=3)
