...
adjusted_phenotypes <- read.csv("Adjusted_Phenotypic_Data_800Lines.csv", header=T)
don_data <- adjusted_phenotypes[,c("line", "cycle", "type", "DON")]
yield_data <- adjusted_phenotypes[,c("line", "cycle", "type", "yld_kg")]
height_data <- adjusted_phenotypes[,c("line", "cycle", "type", "height")]
...
don_data$cycle <- factor(don_data$cycle, levels=c("C0", "C1", "C2", "C3"))
yield_data$cycle <- factor(yield_data$cycle, levels=c("C0", "C1", "C2", "C3"))
height_data$cycle <- factor(height_data$cycle, levels=c("C0", "C1", "C2", "C3"))
...
plt <- ggplot(
    don_data,
    aes(x=factor(cycle), y=DON, fill=factor(type)))
plt + geom_boxplot(position=position_dodge(width=0.75)) +
    ...
    scale_color_manual(
        ...
        name="type") +
    scale_x_discrete(
        breaks=c("C0", "C1", "C2", "C3", "C3"),
        labels=c("Par", "C1", "C2", "C3", "C3")) +
    theme(
...
plt <- ggplot(
    yield_data,
    aes(x=factor(cycle), y=yld_kg, fill=factor(type)))
plt + geom_boxplot(position=position_dodge(width=0.75)) +
    scale_fill_manual(
        values=c("white", "white", "grey", "grey"),
        name="type") +
    ...
    scale_color_manual(
        ...
        name="type") +
    scale_x_discrete(
        breaks=c("C0", "C1", "C2", "C3"),
        labels=c("Par", "C1", "C2", "C3")) +
    theme(
        legend.position="none",
        panel.grid.major=element_blank(),
        panel.grid.minor=element_blank()
...
plt <- ggplot(
    height_data,
    aes(x=factor(cycle), y=height, fill=factor(type)))
plt + geom_boxplot(position=position_dodge(width=0.75)) +
    scale_fill_manual(
        values=c("white", "white", "grey", "grey"),
        name="type") +
